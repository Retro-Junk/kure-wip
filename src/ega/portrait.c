...
int DrawPortrait(byte **desc, byte *x, byte *y, byte *width, byte *height) {
	...
	AddDirtyRect(DirtyRectSprite, cur_image_coords_x, cur_image_coords_y, cur_image_size_w, cur_image_size_h, cur_image_offs, bitofs);
...
void AnimPortrait(byte layer, byte index, byte delay) {
	...
	while (ani != ani_end) {
		...
		GetDirtyRectAndSetSprite(layer, &kind, &x, &y, &width, &height, &offs, &bitofs);
		WaitVBlank();
