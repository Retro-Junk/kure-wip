...
unsigned int DrawPortraitZoomed(byte **params) {
	byte x, y, width, height;
	byte zwidth, zheight;
...
void JaggedZoom(byte *source, byte *target) {
	...
	for (;;) {
		...
		for (i = 0; i < JCOUNT; i++) {
			unsigned int sx = points[i].x;
			unsigned int sy = points[i].y;
			unsigned int ex = points[i + 1].x;
			unsigned int ey = points[i + 1].y;
			if (ex == 0 && ey == 0) {
				ex = points[0].x;
				ey = points[0].y;
			}
...
#else
	TODO("SCR_22_HidePortraitShatter");
#endif
...
Hide a portrait, no special effects
*/
unsigned int SCR_1D_HidePortrait(void) {
...
Hide a portrait, no special effects
*/
unsigned int SCR_21_HidePortrait(void) {
	return SCR_23_HidePortrait();
}
...
Hide a portrait, no special effects
*/
unsigned int SCR_3F_HidePortrait(void) {
	return SCR_23_HidePortrait();
}
...
*/
unsigned int SCR_54_DotFadeRoom(void) {
	script_ptr++;
...
*/
unsigned int SCR_56_MorphRoom98(void) {
	int h;
	unsigned int ofs;
	script_ptr++;
...
*/
unsigned int SCR_F_Unused(void) {
	script_ptr++;
	script_ptr++;
	script_ptr++;
	script_ptr++;
...
Do nothing in PC/CGA version
*/
unsigned int SCR_4A_Unused(void) {
	script_ptr++;
	return 0;
}
...
*/
unsigned int SCR_68_PlaySfx(void) {
	byte index;
	script_ptr++;
	index = *script_ptr++;
	script_ptr++;
...
*/
unsigned int CMD_19_AspirantAppear(void) {
	/*TODO: check me*/
...
Aspirant is dead
*/
unsigned int CMD_1A_AspirantDie(void) {
